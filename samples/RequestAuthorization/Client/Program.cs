    ...
    Host.CreateDefaultBuilder(args)
        .ConfigureServices((hostContext, services) =>
        {
            ...
            services.AddRabbitServiceClient(typeof(ICounterService).Assembly,
                builder => builder.AddFilter(new AddAuthHeaderFilter { Name = "not-a-user" }));
...
class ConsoleServiceHost : IHostedService
{
    ...
    private async Task RunAsync(CancellationToken cancellationToken)
    {
        void PrintUsage()
        {
            ...
            Console.WriteLine("    i: Increment counter by 1.");
            Console.WriteLine("    d: Decrement counter by 1.");
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                switch (cmd)
                {
                    ...
                    case "d":
                        sw.Start();
                        await _counterService.DecrementAsync(cancellationToken);
                        sw.Stop();
                        break;
                    case "v":
                        sw.Start();
